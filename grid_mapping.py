...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  for jy in range(0,nys):
    ...
    for jx in range(0,nxs):
      x1 = vp[0] + xs[jx]   # Instead of vp[0,:]
      y1 = vp[1] + ys[jy]   # Instead of vp[1,:]
...
def update_occupancy_grid(grid, pose, scan, angles, config: 'LidarConfig', map_cfg, debug_file=None):
    sx, sy, syaw = config.sensor_world_pose(pose)
    # print(f"SCAN: pose={pose}, sx={sx:.3f}, sy={sy:.3f}, syaw={syaw:.3f}")

    for r, theta in zip(np.ravel(scan), np.ravel(angles)):
        if r < config.rmin or r > config.rmax_used:
            continue

        # Endpoint in sensor frame
        x_hit = r * np.cos(theta)
        y_hit = r * np.sin(theta)
        # Transform endpoint to world frame
        x_world = sx + x_hit * np.cos(syaw) - y_hit * np.sin(syaw)
        y_world = sy + x_hit * np.sin(syaw) + y_hit * np.cos(syaw)
        # Debug print
        if debug_file is not None:
            debug_file.write(f"sx={sx:.2f}, sy={sy:.2f}, syaw={syaw:.2f}, r={r:.2f}, theta={theta:.2f}, x_hit={x_hit:.2f}, y_hit={y_hit:.2f}, x_world={x_world:.2f}, y_world={y_world:.2f}\n")

        # Convert to grid cells (always scalars)
        ix0, iy0 = map_cfg.meters_to_cells(float(sx), float(sy))
        ix1, iy1 = map_cfg.meters_to_cells(float(x_world), float(y_world))
        # print(f"From ({sx:.2f},{sy:.2f}) to ({x_world:.2f},{y_world:.2f}) → grid ({ix0},{iy0}) to ({ix1},{iy1})")

        # Raytrace for free cells
        cells = bresenham2D(ix0, iy0, ix1, iy1)
        cells_x, cells_y = cells[0], cells[1]

        # Decrease log-odds for free cells
        for cx, cy in zip(cells_x[:-1], cells_y[:-1]):
            cx = int(np.ravel(cx)[0])
            cy = int(np.ravel(cy)[0])
            if 0 <= cx < grid.shape[0] and 0 <= cy < grid.shape[1]:
                # print(f"Updating FREE cell ({cx}, {cy}). Old val: {grid[cx, cy]:.2f}")
                grid[cx, cy] -= 0.4
                grid[cx, cy] = np.clip(grid[cx, cy], -10.0, 10.0)
                # print(f"  New val: {grid[cx, cy]:.2f}")

        # Increase log-odds at hit cell
        cx = int(np.ravel(cells_x[-1])[0])
        cy = int(np.ravel(cells_y[-1])[0])
        if 0 <= cx < grid.shape[0] and 0 <= cy < grid.shape[1]:
            # print(f"Updating OCCUPIED cell ({cx}, {cy}). Old val: {grid[cx, cy]:.2f}")
            grid[cx, cy] += 0.7
            grid[cx, cy] = np.clip(grid[cx, cy], -10.0, 10.0)
            # print(f"  New val: {grid[cx, cy]:.2f}")
